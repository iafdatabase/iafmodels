...
py.arg('--dataset', default='finger_test')
...
py.arg('--epoch_decay', type=int, default=3)  # epoch to start decaying learning rate
...
B_img_paths = py.glob(py.join(args.datasets_dir, args.dataset, 'trainA'), '*.jpg')

for i in range(len(B_img_paths)):
    B_img_paths[i] = B_img_paths[i].replace(".jpg", ".png")
    B_img_paths[i] = B_img_paths[i].replace("trainA", "trainB")
...
with train_summary_writer.as_default():
    for ep in tqdm.trange(args.epochs, desc='Epoch Loop'):
        ...
        for A, B in tqdm.tqdm(A_B_dataset, desc='Inner Epoch Loop', total=len_dataset):
            ...
            # sample
            if G_optimizer.iterations.numpy() % 1000 == 0:
                A, B = next(test_iter)
